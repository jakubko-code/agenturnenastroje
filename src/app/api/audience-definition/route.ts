...
import { generateAudienceDefinition } from "@/server/services/audience-definition-service";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rate = checkRateLimit(`audience-definition:post:${user.id}`, 10, 60_000);
    if (!rate.allowed) {
    ...
    const result = await generateAudienceDefinition({
      userId: user.id,
      model: parsed.data.model,
    ...
    await recordAuditEvent({
      actorUserId: user.id,
      eventType: "tool.audience_definition_generated",
      entityType: "tool_run",
    ...
    if (userId) {
      await recordAuditEvent({
        actorUserId: userId,
        eventType: "tool.audience_definition_generation_failed",
        entityType: "tool_run",
