...
import { generateMetaUniversalTexts } from "@/server/services/meta-universal-service";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rate = checkRateLimit(`meta-universal:post:${user.id}`, 12, 60_000);
    if (!rate.allowed) {
    ...
    const result = await generateMetaUniversalTexts({
      userId: user.id,
      model: parsed.data.model,
    ...
    await recordAuditEvent({
      actorUserId: user.id,
      eventType: "tool.meta_universal_generated",
      entityType: "tool_run",
    ...
    if (userId) {
      await recordAuditEvent({
        actorUserId: userId,
        eventType: "tool.meta_universal_generation_failed",
        entityType: "tool_run",
